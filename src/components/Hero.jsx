import { useState, useEffect } from 'react'
import { useScrollReveal } from '../hooks/useScrollReveal'

function Hero() {
    const { ref, isVisible } = useScrollReveal()
    const [displayText, setDisplayText] = useState('')
    const [currentIndex, setCurrentIndex] = useState(0)
    const [isDeleting, setIsDeleting] = useState(false)
    const [loopNum, setLoopNum] = useState(0)

    const titles = [
        'Web Developer',
        'Software Engineer',
        'Problem Solver',
        'Tech Enthusiast'
    ]

    useEffect(() => {
        const currentTitle = titles[loopNum % titles.length]

        const timeout = setTimeout(() => {
            if (!isDeleting) {
                setDisplayText(currentTitle.substring(0, currentIndex + 1))
                setCurrentIndex(prev => prev + 1)

                if (currentIndex === currentTitle.length) {
                    setTimeout(() => setIsDeleting(true), 2000)
                }
            } else {
                setDisplayText(currentTitle.substring(0, currentIndex - 1))
                setCurrentIndex(prev => prev - 1)

                if (currentIndex === 0) {
                    setIsDeleting(false)
                    setLoopNum(prev => prev + 1)
                }
            }
        }, isDeleting ? 50 : 100)

        return () => clearTimeout(timeout)
    }, [currentIndex, isDeleting, loopNum, titles])

    return (
        <section
            id="home"
            ref={ref}
            className={`hero-section min-h-screen flex items-center relative overflow-hidden transition-all duration-700 ${isVisible ? 'opacity-100' : 'opacity-0'}`}
        >
            <div className="container mx-auto px-6 lg:px-12 relative z-10">
                <div className="grid lg:grid-cols-2 gap-12 items-center">
                    {/* Left Content */}
                    <div className="hero-content">
                        {/* Greeting */}
                        <p className="text-lg md:text-xl text-[#1B263B] mb-2 font-medium">
                            Hello Buds
                        </p>

                        {/* Name */}
                        <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-2">
                            <span className="text-[var(--primary)]">I am </span>
                            <span className="text-[var(--accent)] hover:text-[var(--accent-dark)] transition-colors">Nama Anda</span>
                        </h1>

                        {/* Subtitle with Typewriter Effect */}
                        <p className="text-xl md:text-2xl font-bold mb-6 h-8">
                            <span className="text-[var(--primary)] opacity-60">I'm a </span>
                            <span className="text-[var(--accent)] border-r-2 border-[var(--accent)] pr-1 animate-pulse">
                                {displayText}
                            </span>
                        </p>

                        {/* Divider */}
                        <div className="w-16 h-1 bg-[#1B263B] mb-6"></div>

                        {/* Description */}
                        <p className="text-base md:text-lg text-[var(--primary)]/70 max-w-lg mb-8 leading-relaxed">
                            I'm a person who has a keen interest in the design layout. I think presenting
                            an attractive design is a matter of concern in developing a branding of products.
                            To creates a good design, I focus on proper composition and visual decoration
                            details in order to make it more professional. For the time being, I'm developing
                            the skill for acquiring the UI/UX design for dynamic application and web development.
                        </p>

                        {/* CTA Buttons */}
                        <div className="flex flex-wrap gap-4 mb-8">
                            <a href="#" className="btn-download inline-flex items-center gap-2">
                                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                </svg>
                                Download CV
                            </a>
                            <a href="#about" className="btn-more">
                                More
                            </a>
                        </div>

                        {/* Social Links */}
                        <div className="flex items-center gap-4 mt-8">
                            <span className="text-[#1B263B] font-medium">Find Me On</span>
                            <div className="flex gap-3">
                                <a href="#" className="social-circle">
                                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                        <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                                    </svg>
                                </a>
                                <a href="#" className="social-circle">
                                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                                    </svg>
                                </a>
                                <a href="#" className="social-circle">
                                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                                    </svg>
                                </a>
                                <a href="#" className="social-circle">
                                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                        <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
                                    </svg>
                                </a>
                            </div>
                        </div>
                    </div>

                    {/* Right Content - Profile Image */}
                    <div className="hero-image-container relative flex justify-center lg:justify-end">
                        {/* Decorative Elements */}
                        <div className="absolute top-10 right-20 floating">
                            <svg className="w-6 h-6 text-[#60A5FA]" fill="currentColor" viewBox="0 0 20 20">
                                <path d="M10 3a1 1 0 011 1v5h5a1 1 0 110 2h-5v5a1 1 0 11-2 0v-5H4a1 1 0 110-2h5V4a1 1 0 011-1z" />
                            </svg>
                        </div>
                        <div className="absolute top-1/3 right-10 floating" style={{ animationDelay: '1s' }}>
                            <div className="w-8 h-8 border-2 border-[#60A5FA]/50 rounded-full"></div>
                        </div>

                        {/* Profile Image with Decorative Background */}
                        <div className="profile-image-wrapper relative">
                            {/* Background Shape */}
                            <div className="absolute inset-0 bg-[#1B263B] rounded-tl-[100px] rounded-tr-[100px] rounded-bl-[40px] rounded-br-[40px] transform translate-x-4 translate-y-4"></div>

                            {/* Main Image */}
                            <div className="relative z-10 overflow-hidden rounded-tl-[100px] rounded-tr-[100px] rounded-bl-[40px] rounded-br-[40px]">
                                <img
                                    src="/images/profile.png"
                                    alt="Profile"
                                    className="w-80 h-auto object-cover profile-image"
                                />
                                {/* Blue Overlay Effect */}
                                <div className="absolute inset-0 bg-gradient-to-t from-[#1B263B]/30 to-transparent mix-blend-multiply"></div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
    )
}

export default Hero
